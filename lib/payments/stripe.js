...
// Demo toggle: when true, we bypass Stripe calls entirely.
const DEMO = process.env.DEMO_NO_PAYMENT === 'true';
...
router.post('/setup', async (req, res, next) => {
    ...
    // const stripe = require('stripe')(stripeConfig.secretKey);
        ...
        const stripeConfig = getPaymentConfig('stripe');
        if (DEMO) {
            // Fake a "customer" and "clientSecret" so the frontend can proceed.
            req.session.stripeCustomer = 'cus_demo_' + Date.now();
            return res.send({
...
router.get('/checkout_action', async (req, res, next) => {
    ...
    const stripeConfig = getPaymentConfig('stripe');
    // const stripe = require('stripe')(stripeConfig.secretKey);
    const stripe = DEMO ? null : require('stripe')(stripeConfig.secretKey);
    ...
    // );
        ...
        let paymentIntent;
        if (DEMO) {
            // Fabricate a successful payment intent for demo purposes
            paymentIntent = {
                id: 'pi_demo_' + Date.now(),
                status: 'succeeded',
                metadata: {
                    paymentType: req.session.cartSubscription ? 'subscription' : 'single'
                },
                payment_method: 'pm_demo_123',
                customer: req.session.stripeCustomer || ('cus_demo_' + Date.now())
            };
        } else {
            // Check for the result
            if(!result || !result.payment_intent){
            ...
            paymentIntent = await stripe.paymentIntents.retrieve(result.payment_intent);
        }
    ...
    if(!DEMO && orderDoc.orderType === 'subscription'){
